class ApplyBeverageImagesStrategy {
    static async execute(beverages) {
        const baseUrl = `http://localhost:${process.env.APP_APPLICATION_PORT}/images`;
        ...
        const beveragePromises = beverages.map(async beverage => {
            const imageUrl = `${baseUrl}/beverage_${beverage.id}.png`;
            try {
                const response = await fetch(imageUrl);
                if (!response.ok) throw new Error('Image not found');
                return { ...beverage, imageUrl: imageUrl };
            } catch (error) {
                return { ...beverage, imageUrl: defaultImage };
            }
        });
